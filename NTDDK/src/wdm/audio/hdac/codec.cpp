...
// Pin Widget Control bits (практично достаточно этого)
#define PINCTL_OUT_EN                 0x40
#define PINCTL_IN_EN                  0x20

BOOLEAN dumpPins = TRUE; // временно
...
void HDA_Codec::ForceEapd(ULONG pinNid, BOOLEAN enable)
{
    ...
    // Обычно 0x02 (EAPD) или 0x03 (EAPD+BTL). Для ALC662 часто безопасно 0x02.
...
void HDA_Codec::SwitchOutput(BOOLEAN hpPresent)
{
    ...
    if (hpPresent)
    {
		...
		DbgPrint("WDMHDA: COEF snapshot HEADPHONES\n");
		DumpCoefs(32, 0x00, 0x3F);
		...
		// сначала mute/unmute пинов
		MutePinOutAmp(speakerPin);
		UnmutePinOutAmp(hpPin);

        // ВАЖНО: чтобы дальнейшие регуляторы/рутинги в драйвере работали по “активному” пину
    ...
    else
    {
		...
		DbgPrint("WDMHDA: COEF snapshot SPEAKERS\n");
		DumpCoefs(32, 0x00, 0x3F);
	...
	DumpPinState(2);   // DAC2
	DumpPinState(12);  // mixer12
	DumpPinState(20);  // speaker pin
	DumpPinState(27);  // hp pin
}

void HDA_Codec::ForcePlaybackChain()
{
    // 1) DAC2 (NID 2) output amp: UNMUTE + gain=0 (как AMP_OUT_ZERO в ALSA логике)
    SetOutAmpLR(2, FALSE, 0x00);

    // 2) Mixer12: вход от DAC (idx=0) + выход mixer: UNMUTE
    // Важно: для IN amp тоже лучше сделать LR отдельно, но начнем с одного (обычно влияет на оба).
    UnmuteInAmp(12, 0, 0x00);
    UnmuteOutAmp(12, 0x00);

    // 3) На всякий случай: selector mixer12 -> DAC
    ForceConnSel(12, 0);
}
...
void HDA_Codec::StartHpPolling()
{
    ...
    // стартовое состояние: подхватим текущее и применим
    HpPrev = IsHpPresent();
    SwitchOutput(HpPrev);
...
void HDA_Codec::StopHpPolling()
{
    ...
    // 1) отменяем будущие тики
    BOOLEAN wasSet = KeCancelTimer(&HpTimer);
    KeRemoveQueueDpc(&HpDpc);

    // 3) ждём, пока выполняющийся DPC (если он в полёте) закончит
    // Event выставляется в конце DPC
    LARGE_INTEGER timeout;
    timeout.QuadPart = -10 * 1000 * 1000; // 1 секунда (в 100-нс), отрицательное = relative
...
static ULONG amp_get_cmd(BOOLEAN output, BOOLEAN right, UCHAR index)
{
    ...
    cmd |= right  ? 0x2000 : 0x1000;     // R или L
...
static ULONG amp_set_cmd(BOOLEAN output, BOOLEAN right, UCHAR index, BOOLEAN mute, UCHAR gain)
{
    ULONG cmd = 0;
    cmd |= output ? 0x8000 : 0x0000;
    cmd |= right  ? 0x2000 : 0x1000;   // выбери канал
    cmd |= ((ULONG)(index & 0x1F)) << 8;
    if (mute) cmd |= 0x0080;
    cmd |= (gain & 0x7F);
    return cmd;
...
void HDA_Codec::DumpConnList(ULONG nid)
{
	if (InShutdown) return;

    ULONG len = hda_send_verb(nid, VERB_GET_CONN_LIST_LEN, AC_PAR_CONNLIST_LEN);
    UCHAR nEntries = (UCHAR)(len & 0x7F);
    BOOLEAN longForm = (len & 0x80) ? TRUE : FALSE;

    hda_log("WDMHDA: ConnList nid=%lu len=0x%08lX entries=%u long=%u\n",
            nid, len, (ULONG)nEntries, longForm ? 1UL : 0UL);

    // читаем по 4 short entries за раз (если short form), или по 2 long entries за раз
    // для простоты: читаем первые 16 индексов максимум
    for (UCHAR idx = 0; idx < nEntries && idx < 16; )
    {
        ULONG resp = hda_send_verb(nid, VERB_GET_CONN_LIST_ENTRY, idx);
        hda_log("WDMHDA:   idx=%u resp=0x%08lX\n", (ULONG)idx, resp);
...
void HDA_Codec::WakeSpeakerPath()
{
    ...
    // 1) Для спикеров логично брать вход idx=0, который у тебя NID=2 (DAC 2)
    // (idx=1 ведёт на NID 11, часто это “sum/loop/mixer” и может быть замьючен)
    ForceConnSel(mix12, 0);

    // 2) Снимаем mute на микшере 12: на входе 0 и на выходе
    UnmutePinInAmp(mix12, 0);
    UnmutePinOutAmp(mix12);

    // 3) На самом спикерном пине тоже оставляем OUT amp unmute (ты уже сделал)
    UnmutePinOutAmp(speakerPin);

    // 4) EAPD иногда нужно 0x03 (EAPD+BTL). Попробуем более агрессивно.
...
void HDA_Codec::DumpCoefs(ULONG node, USHORT from, USHORT to)
{
    DbgPrint("WDMHDA: ---- COEF dump node=%lu idx=%u..%u ----\n", node, (ULONG)from, (ULONG)to);

    for (USHORT i = from; i <= to; i++)
    {
        hda_send_verb(node, 0x500, i);        // SET_COEF_INDEX
        ULONG v = hda_send_verb(node, 0xC00, 0); // GET_PROC_COEF

        // пропускаем нули, чтобы не спамить
        if ((v & 0xFFFF) != 0x0000)
            DbgPrint("WDMHDA: COEF[%02X]=%04lX\n", (ULONG)i, (v & 0xFFFF));
    }

    DbgPrint("WDMHDA: ---- COEF dump end ----\n");
}

void HDA_Codec::ApplyEeeInit()
{
    // 1) Пины 0x14 (20) и 0x1b (27): PIN_OUT + AMP_OUT_UNMUTE + CONNECT_SEL=0
    hda_log("WDMHDA: ApplyEeeInit\n");

    // PIN_OUT: в ALSA это обычно 0x40 (только OUT_EN). Иногда 0xC0, но начнём как у них.
    hda_send_verb(20, 0x707, 0x40);
    hda_send_verb(27, 0x707, 0x40);

    // CONNECT_SEL на самих пинах!
    hda_send_verb(20, 0x701, 0x00);
    hda_send_verb(27, 0x701, 0x00);

    // AMP_OUT_UNMUTE на пинах (и важно: L и R отдельно)
    SetOutAmpLR(20, FALSE, 0x40);
    SetOutAmpLR(27, FALSE, 0x40);

    // 2) DAC 0x02/0x03/0x04 AMP_OUT_ZERO (как в патче)
    // AMP_OUT_ZERO: mute=0? в ALSA это обычно "не mute, gain=0"
    // Мы сделаем gain=0 и не mute на обоих каналах:
    SetOutAmpLR(2, FALSE, 0x00);
    SetOutAmpLR(3, FALSE, 0x00);
    SetOutAmpLR(4, FALSE, 0x00);
}

void HDA_Codec::DumpPinState(ULONG nid)
{
    ULONG pctl = hda_send_verb(nid, VERB_GET_PIN_WIDGET_CONTROL, 0);
    ULONG eapd = hda_send_verb(nid, VERB_GET_EAPD_BTLENABLE, 0);
    ULONG al   = hda_send_verb(nid, VERB_GET_AMP_GAIN_MUTE, amp_get_cmd(TRUE, FALSE, 0));
    ULONG ar   = hda_send_verb(nid, VERB_GET_AMP_GAIN_MUTE, amp_get_cmd(TRUE, TRUE,  0));

    hda_log("WDMHDA: NID %lu PINCTL=0x%08lX EAPD=0x%08lX AMP_L=0x%08lX AMP_R=0x%08lX\n",
            nid, pctl, eapd, al, ar);
}
// Scht. <<<
...
STDMETHODIMP_(NTSTATUS) HDA_Codec::hda_initialize_audio_function_group(ULONG afg_node_number) {
	...
	for (ULONG node = ((subordinate_node_count_reponse>>16) & 0xFF), 
		...
		if(type_of_node == HDA_WIDGET_AUDIO_OUTPUT) {
			...
			if (dumpPins)
			{
				ULONG pincap = hda_send_verb(node, 0xF00, 0x0C);   // Pin Capabilities
				ULONG psense = hda_send_verb(node, 0xF09, 0x00);   // Pin Sense
				ULONG pctl   = hda_send_verb(node, 0xF07, 0x00);   // Pin Widget Control

				DbgPrint("\nPINDBG nid=%lu cfg=0x%08lX pincap=0x%08lX pctl=0x%08lX sense=0x%08lX present=%lu",
						node, pin_config, pincap, pctl, psense, (psense & 0x80000000) ? 1 : 0);
			}
			// Scht. <<<<<<
		...
		//if it's not a PIN then what other type of node is it?
			...
			// Scht. >>>>>>>>>>>>>
			__try {
				ULONG c0 = ReadCoef(node, 0x00);
				ULONG c1 = ReadCoef(node, 0x01);
				ULONG c10 = ReadCoef(node, 0x10);
				hda_log("WDMHDA: COEF probe node=%lu c00=%04lX c01=%04lX c10=%04lX\n",
						 node, (c0 & 0xFFFF), (c1 & 0xFFFF), (c10 & 0xFFFF));
			} __except(EXCEPTION_EXECUTE_HANDLER) {
				hda_log("WDMHDA: COEF probe node=%lu failed\n", node);
			}
			// Scht. <<<<<<<<<<<<<
		} else {
